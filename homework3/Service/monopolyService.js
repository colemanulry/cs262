...
router.get("/playersGame", readPlayersGame);
...
function readPlayersGame(req, res, next) {
    db.many("SELECT score FROM Player, PlayerGame WHERE Player.ID = PlayerGame.playerID AND Player.emailAddress = 'me@calvin.edu'")
    .then(data => {
            res.send(data);
        })
        .catch(err => {
